...
#                         if current_game_id and 'message' and current_player in message:
#                             message = message['message']
#                             ChatMessage.objects.create(game=current_game_id, player=current_player, message=message)
...
#     try:
#         history = ChatMessage.objects.filter(game=game_id).order_by('timestamp')[:50] # Get last 50 messages
#         history_messages = [{"player": msg.player.player_import.username, "message": msg.message, "time_sent": str(msg.timestamp)} for msg in history]
                ...
                try:
                    ...
                    if message_type == 'join_room':
                        ...
                        if current_game_id and 'message' in message and current_player :
                            message = message['message']
                            ChatMessage.objects.create(game_import=current_game_id, message_sender=current_player, message=message)
                            broadcast(current_game_id, f"{current_player.player_import.username}: {message}", client_socket)
                    elif message_type == 'leave_room':
                        if current_game_id and client_socket in CHAT_ROOMS.get(current_game_id, []):
                            CHAT_ROOMS[current_game_id].remove(client_socket)
...
def send_chat_history(client_socket, game_id):
    try:
        history = ChatMessage.objects.filter(game_import=game_id).order_by('timestamp')[:50] # Get last 50 messages
        history_messages = [{"player": msg.player.player_import.username, "message": msg.message, "time_sent": str(msg.timestamp)} for msg in history]
        send_websocket_message(client_socket, json.dumps({'type': 'history', 'messages': history_messages}))
    except Exception as e:
        print(f"Error sending chat history for game {game_id}: {e}")
